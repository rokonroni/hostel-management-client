...
const MealDetails = () => {
  ...
  const { data: mealDetails, isLoading, isError, refetch } = useQuery({
    queryKey:["meal", id],
    queryFn: async () => {
...
// const dateOnlyString = dateTime.toISOString().split('T')[0];
  ...
  const handleLikeClick = async () => {
    if (user) {
      try {
      const response = await axiosPublic.post(`/meal/like/${id}`);
  ...
  if (isError) {
    return <div className="text-center mt-8 text-red-500">Error loading meal details</div>;
  }
  ...
  return (
    <div className="container mx-auto my-8 p-4 bg-white rounded-lg shadow-lg">
      ...
      {mealDetails && (
        <>
          <img src={mealDetails.meal_image} className="w-full rounded-md mb-4" />
          <h2 className="text-2xl font-semibold mb-2">{mealDetails.admin_name}</h2>
          <p className="text-gray-600 mb-4">{mealDetails.description}</p>
        <p className="text-gray-700 mb-2">
          Ingredients: {mealDetails.ingredients}
        </p>
        <p className="text-gray-700 mb-2">Post Time: {mealDetails.time_date}</p>
